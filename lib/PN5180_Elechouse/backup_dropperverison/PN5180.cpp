...
static const bool kPn5180IoVerbose = false;

static void logSpiTimeout(const __FlashStringHelper *phase, unsigned long elapsed, int busy, uint8_t cmd, uint8_t arg) {
  Serial.print(F("PN5180 SPI timeout "));
  Serial.print(phase);
...
PN5180::PN5180(uint8_t SSpin, uint8_t BUSYpin, uint8_t RSTpin, SPIClass& spi) :
  ...
  PN5180_RST(RSTpin),
  PN5180_CE(-1),
  PN5180_CE_activeHigh(true),
  autoHardResetOnTimeout(false),
  spiTimeoutSeen(false),
  PN5180_SPI(spi),
  PN5180_SCK(-1), 
  PN5180_MISO(-1),
  PN5180_MOSI(-1)
...
void PN5180::begin(int8_t sck, int8_t miso, int8_t mosi, int8_t ss) {
  ...
  if (PN5180_CE >= 0) {
    ...
    digitalWrite(PN5180_CE, PN5180_CE_activeHigh ? HIGH : LOW);
  }
...
void PN5180::setHardResetPin(int8_t cePin, bool activeHigh) {
  PN5180_CE = cePin;
  PN5180_CE_activeHigh = activeHigh;
  if (PN5180_CE >= 0) {
    pinMode(PN5180_CE, OUTPUT);
    digitalWrite(PN5180_CE, PN5180_CE_activeHigh ? HIGH : LOW);
  }
...
void PN5180::setAutoHardResetOnTimeout(bool enable) {
  autoHardResetOnTimeout = enable;
}
...
void PN5180::hardReset() {
  ...
  int disableLevel = PN5180_CE_activeHigh ? LOW : HIGH;
  int enableLevel = PN5180_CE_activeHigh ? HIGH : LOW;
  digitalWrite(PN5180_CE, disableLevel);
  delay(10);
  digitalWrite(PN5180_CE, enableLevel);
  delay(50);
}

bool PN5180::getAndClearSpiTimeout() {
  bool seen = spiTimeoutSeen;
  spiTimeoutSeen = false;
  return seen;
}
...
bool PN5180::readRegister(uint8_t reg, uint32_t *value) {
  ...
  bool ret = transceiveCommand(cmd, sizeof(cmd), (uint8_t*)value, 4);
  if (!ret) {
    *value = 0;
    PN5180DEBUG_EXIT;
    return false;
...
bool PN5180::sendData(const uint8_t *data, int len, uint8_t validBits) {
  ...
  if (kPn5180IoVerbose) {
    Serial.print(F("PN5180 sendData len="));
    Serial.print(len);
  ...
  if (kPn5180IoVerbose) {
    Serial.print(F("PN5180 sendData state="));
    Serial.println((int)transceiveState);
  }
...
uint8_t * PN5180::readData(int len) {
  ...
  if (kPn5180IoVerbose) {
    Serial.print(F("PN5180 readData len="));
    Serial.println(len);
  }
...
bool PN5180::readData(int len, uint8_t *buffer) {
  ...
  if (kPn5180IoVerbose) {
    Serial.print(F("PN5180 readData(buf) len="));
    Serial.println(len);
  }
...
bool PN5180::transceiveCommand(uint8_t *sendBuffer, size_t sendBufferLen, uint8_t *recvBuffer, size_t recvBufferLen) {
  ...
  uint8_t cmd = (sendBufferLen > 0) ? sendBuffer[0] : 0;
  uint8_t arg = (sendBufferLen > 1) ? sendBuffer[1] : 0;
#ifdef DEBUG
  ...
  while (LOW != digitalRead(PN5180_BUSY)) {
	  ...
	  if (millis() - startedWaiting > commandTimeout) {
		  logSpiTimeout(F("send/0"), millis() - startedWaiting, digitalRead(PN5180_BUSY), cmd, arg);
  ...
  while (HIGH != digitalRead(PN5180_BUSY)) {
	  delay(1);
	  if (millis() - startedWaiting > commandTimeout) {
		  logSpiTimeout(F("send/3"), millis() - startedWaiting, digitalRead(PN5180_BUSY), cmd, arg);
  ...
  while (LOW != digitalRead(PN5180_BUSY)) {
	  delay(1);
	  if (millis() - startedWaiting > commandTimeout) {
		  logSpiTimeout(F("send/5"), millis() - startedWaiting, digitalRead(PN5180_BUSY), cmd, arg);
  ...
  while (HIGH != digitalRead(PN5180_BUSY)) {
	  delay(1);
	  if (millis() - startedWaiting > commandTimeout) {
		  logSpiTimeout(F("receive/3"), millis() - startedWaiting, digitalRead(PN5180_BUSY), cmd, arg);
  ...
  while (LOW != digitalRead(PN5180_BUSY)) {
	  delay(1);
	  if (millis() - startedWaiting > commandTimeout) {
		  logSpiTimeout(F("receive/5"), millis() - startedWaiting, digitalRead(PN5180_BUSY), cmd, arg);
...
void PN5180::handleSpiTimeout() {
  spiTimeoutSeen = true;
  if (autoHardResetOnTimeout && (PN5180_CE >= 0)) {
    hardReset();
  }
}
