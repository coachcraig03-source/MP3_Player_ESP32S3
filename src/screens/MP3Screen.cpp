...
void MP3Screen::loadAlbumsFromSD() {
    ...
    FsFile root;
    if (!root.open("/")) {
        Serial.println("MP3Screen: Failed to open root");
        return;
    }
...
void MP3Screen::drawAlbumArt() {
    ...
    char artPath[128];
    snprintf(artPath, sizeof(artPath), "/%s/folder.jpg", albumNames[selectedAlbum]);
...
void MP3Screen::selectAlbum(int index) {
    ...
    char folderPath[128];
    snprintf(folderPath, sizeof(folderPath), "/%s", albumNames[index]);
...
void MP3Screen::playTrack(int index) {
    ...
    // Build full path: /AlbumName/TrackName.mp3
    char trackPath[256];
